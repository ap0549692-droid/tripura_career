...
class FetchGovtJobs extends Command
{
    ...
    'Referer' => 'https://tpsc.tripura.gov.in/',
            ...
            $crawler->filter('a')->each(function (Crawler $node) use (&$count) {
                ...
                if($job->wasRecentlyCreated){
                    ...
                    $posterPath = $this->generateJobPoster($job);
    ...
    private function generateJobPoster($job)
    {
        ...
        for($y=0; $y<$H; $y++){
            $r = (int)(11 + $y*0.15); $g = (int)(35 + $y*0.08); $b = (int)(84 + $y*0.05);
        ...
        if($fontBold) imagettftext($img, 58, 0, 215, 155, $white, $fontBold, "TRIPURA CAREER");
        if($fontBold) imagettftext($img, 26, 0, 350, 225, $lightGray, $fontBold, "GOVT JOB ALERT 2026");
    ...
    private function postToFacebook($job, $posterPath = null){
        try {
            ...
            if($posterPath && file_exists($posterPath)){
                $response = Http::attach('source', file_get_contents($posterPath), $job->id.'.jpg')->post("https://graph.facebook.com/v26.0/".env('FB_PAGE_ID')."/photos", ['caption' => $message, 'access_token' => env('FB_PAGE_TOKEN')]);
            } else {
                $response = Http::post("https://graph.facebook.com/v26.0/".env('FB_PAGE_ID')."/feed", ['message' => $message, 'access_token' => env('FB_PAGE_TOKEN')]);
            }
            Log::info("FB Response: ".$response->body());
            $this->info("-> FB Posted");
        } catch (\Exception $e) { $this->error("FB Failed: ".$e->getMessage()); }
    }
    ...
    private function postToInstagram($job, $localPosterPath){
        try {
            ...
            if (!$igUserId ||!$token){ $this->error("-> IG Error: IG_USER_ID ya Token.env me nahi hai"); return; }
            if(!file_exists($localPosterPath)){ $this->error("-> IG Error: Poster file nahi mila"); return; }
            $this->info("-> Uploading poster to public link...");
            $resPublic = Http::attach('fileToUpload', file_get_contents($localPosterPath), $job->id.'.jpg')->post('https://catbox.moe/user/api.php', ['reqtype' => 'fileupload']);
            if(!$resPublic->successful()){ $this->error("-> Public Upload Failed: ".$resPublic->body()); return; }
            $publicUrl = trim($resPublic->body());
            $this->info("-> Public URL: ".$publicUrl);
            $caption = "🔥 ".$job->title."\n\nDepartment: ".$job->department."\nLast Date: ".date('d M Y', strtotime($job->last_date))."\n\nApply: tripuracareer.com (Link in Bio)\n\n#TripuraJobs #GovtJobs #TPSC #TripuraCareer";
            $res1 = Http::post("https://graph.facebook.com/v26.0/{$igUserId}/media", ['image_url' => $publicUrl, 'caption' => $caption, 'access_token' => $token]);
            if (!$res1->successful()) { $this->error("-> IG Container: ".$res1->body()); return; }
            $creationId = $res1->json()['id'];
            sleep(6);
            $res2 = Http::post("https://graph.facebook.com/v26.0/{$igUserId}/media_publish", ['creation_id' => $creationId, 'access_token' => $token]);
